{
    ...
    //  - R6 - previous redemptions under AVL+ tree
    //
    //
    // Actions:
    //  - redeem note (#0)
    ...
    //
    // Redemption:
    // to redeem, ownership tree should be presented. Tree contains reserveId as a key,
    // signature as value, and message signed is current position in the tree, note value and note id.
    // note_id is to be added to redeemed notes. Receipt is to be created as well, see receipt contract.
    //
    // Re-redemption:
    //
    //
    ...
    if (action == 0) {
      ...
      // todo: fix ids
      val history = getVar[AvlTree](0).get
      val redeemPosition = getVar[Long](0).get
      ...
      // checking tree proofs of inclusion for redemption leaf
      val rLeafKeyBytes = longToByteArray(redeemPosition)
      ...
      // top up
      sigmaProp(selfPreserved && selfOut.R6[AvlTree] == SELF.R6[AvlTree]
      ...
      if (action == 2) {
        ...
        sigmaProp(selfPreserved && selfOut.R6[AvlTree] == SELF.R6[AvlTree] && correctHeight) && proveDlog(ownerKey)
      } else if (action == 3) {
        // cancel refund
        ...
        // complete refund
        val refundNotificationPeriod = 72000 // 100 days
